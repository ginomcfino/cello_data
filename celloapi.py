...
class CelloQuery:
    ...
    def get_results(self) -> int:
        ...
        # properly.
        (clobber, last_run_name) = self.check_for_prior_results()
        if clobber:
            if self.archival:
                print("Prior results detected, moving to prevent clobber.")
                dest = self.archive_prior_results(last_run_name, '123')
    ...
    def check_for_prior_results(self) -> bool:
        ...
        for c in dir_contents:
            ...
            # if '_' in f_name:
            #     v_name = f_name.split('_')[:-1]
            #     v_name = '_'.join(v_name)
            # else:
            #     v_name = f_name
            names.append(f_name)
            # print()
        ...
        if len(names) > 1:
            v_name = Counter(names).most_common(1)[0][0]
        else:
            v_name = 'generic'
    ...
    def archive_prior_results(self, v_name: str, ucf_name: str) -> str:
        """
        ...
        archive_dir_name = f"({v_name})_{ucf_name}_prior_cello_result_{time_string}"
